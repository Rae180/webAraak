...
import 'package:start/features/Resources/view/Screens/AddNewWoodScreen.dart'; // Import your wood model

class AllWoodScreen extends StatefulWidget {
  static const String routeName = '/all-wood';

  const AllWoodScreen({super.key});

  @override
  State<AllWoodScreen> createState() => _AllWoodScreenState();
}

class _AllWoodScreenState extends State<AllWoodScreen> {
  late ResourcesBloc _resourcesBloc;
  bool _woodLoaded = false;
  ...
  void _handleRefresh() {
    _resourcesBloc.add(GetAllWoodEvent());
    setState(() {
      _woodLoaded = false;
    });
  }

  void _showEditWoodDialog(BuildContext context, Wood wood) {
    final _formKey = GlobalKey<FormState>();
    final _priceController =
        TextEditingController(text: wood.pricePerMeter?.toString() ?? '');
      ...
      builder: (BuildContext context) {
        return BlocListener<ResourcesBloc, ResourcesState>(
          listener: (context, state) {
            if (state is EditWoodSuccess) {
              Navigator.of(context).pop();
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(
                    'تم تعديل سعر الخشب بنجاح',
    ...
    return BlocProvider.value(
      ...
      child: Scaffold(
        ...
        floatingActionButton: FloatingActionButton(
          onPressed: () {
            // Navigate to AddNewWoodScreen
            Navigator.of(context).push(
              MaterialPageRoute(
                builder: (context) => BlocProvider.value(
                  value: _resourcesBloc,
                  child: AddNewWoodScreen(),
                ),
              ),
            );
          },
  ...
  Widget _buildBody(BuildContext context, Color textColor) {
    return BlocConsumer<ResourcesBloc, ResourcesState>(
      ...
      builder: (context, state) {
        ...
        if (state is GetAllWoodSuccess) {
          return _buildWoodListView(state.woods, textColor);
        }
        if (state is ResourcesError) {
          return _buildErrorWidget(state.message, textColor);
        }
        // Initial state - load wood
        if (!_woodLoaded) {
  ...
  Widget _buildWoodListView(AllWoodModel woods, Color textColor) {
    if (woods.wood == null || woods.wood!.isEmpty) {
      return Center(
        child: Text(
          'لا يوجد أنواع خشب حالياً',
          style: TextStyle(
            color: textColor,
            fontFamily: AppConstants.primaryFont,
          ),
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemCount: woods.wood!.length,
        itemBuilder: (context, index) {
          final wood = woods.wood![index];
          return Container(
              ...
              trailing: Row(
                ...
                children: [
                  IconButton(
                    ...
                    onPressed: () {
                      _showEditWoodDialog(context, wood);
                    },
                  ),
                  IconButton(
                    icon: Icon(
                    ...
                    onPressed: () =>
                        _showDeleteWoodConfirmationDialog(context, wood),
                  ),
                ],
              ),
            ),
  ...
  void _showDeleteWoodConfirmationDialog(BuildContext context, Wood wood) {
    showDialog(
      context: context,
      builder: (BuildContext dialogContext) {
        return AlertDialog(
          ...
          content: Text(
            'هل أنت متأكد من أنك تريد حذف هذا النوع من الخشب؟',
            style: TextStyle(
              fontFamily: AppConstants.primaryFont,
            ),
          ),
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                // Add delete wood event
                _resourcesBloc.add(DeleteWoodEvent(id: wood.id!));
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(
                      'سيتم حذف نوع الخشب قريباً',
                      style: TextStyle(fontFamily: AppConstants.primaryFont),
                    ),
                  ),
                );
